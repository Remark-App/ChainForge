...
const PromptNode = ({ data, id, type: node_type }) => {
  ...
  // Chat node specific
  const [contChatWithPriorLLMs, setContChatWithPriorLLMs] = useState(
    data.contChat !== undefined ? data.contChat : true
  );
  ...
  const refreshTemplateHooks = (text) => {
    ...
    const found_template_vars = Array.from(
      new Set(extractBracketedSubstrings(text))
  ...
  const handleRunClick = (event) => {
    ...
    console.log('==pulled_data==', pulled_data, "==templateVars==", templateVars);
    ...
    const query_llms = () => {
      return fetch_from_backend(
        ...
        if (!json) {
          ...
          if (responsesWillChange) setUninspectedResponses(true);
          setResponsesWillChange(false);
    ...
    fetch_resp_count()
      ...
      .then(() => {
        if (Object.keys(pulled_data).length > 0 && Object.keys(pulled_data).length === templateVars.length ) {
          query_llms();
        } else {
          // 获取所有 tmpPrompt
          ...
          if (edges) {
            for (let i = 0; i < edges.length; i++) {
              if (edges[i].target === id) {
                console.log(
                  "==getNode(edges[i].source)==",
                  getNode(edges[i].source),
                  "==getNode(edges[i].target)==",
                  getNode(edges[i].target),
                  "edges[i].targetHandle==",
                  edges[i].targetHandle
                );
                ...
                if (getNode(edges[i].source).type === "evaluator") {
                  ...
                  console.log(
                    "=data=json",
                    StorageCacheData[edges[i].source + ".json"]
                  );
                  ...
                  for (let m = 0; m < tmpCacheData.length; m++) {
                    for (
                      let n = 0;
                      n < tmpCacheData[m]["eval_res"]["items"].length;
                      n++
                    ) {
                      tmpArray.push(
                        JSON.stringify(tmpCacheData[m]["eval_res"]["items"][n])
                      );
                      tmpPromptToArray.push(
                        JSON.stringify(tmpCacheData[m]["eval_res"]["items"][n])
                      );
                    }
                  }
          ...
          if (getNode(id).data && getNode(id).data.prompt) {
            ...
            pulled_data = {...pulled_data, ...promptAndArray };
            console.log("==pulled_data==", pulled_data);
      ...
      <div>
        ...
        {node_type === "chat" ? (
          <div>
            <Switch
              label={
                contChatWithPriorLLMs
                  ? "Continue chat with prior LLM(s)"
                  : "Continue chat with new LLMs:"
              }
              defaultChecked={true}
