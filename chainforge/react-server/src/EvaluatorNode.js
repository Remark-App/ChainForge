...
const EvaluatorNode = ({ data, id }) => {
  ...
  const handleRunClick = (event) => {
    ...
    fetch_from_backend(execute_route, {
      ...
      .then(function (json) {
        ...
        console.log('==run=script=result==', json.responses);
        setLastResponses(json.responses);
  ...
  useEffect(() => {
    console.log("=lastResponsed=", lastResponses);

    // if (lastResponses.length > 0) {
    //   let newLastResponsed = [...lastResponses];

    //   let beChanged = false;
    //   for (let i = 0; i < newLastResponsed; i++) {
    //     if (Object.keys(newLastResponsed[i].vars).length === 0) {
    //       newLastResponsed[i].vars = lastResponses[i].eval_res;
    ...
    //   if (beChanged === true) {
    //     setLastResponses(newLastResponsed);
    //   }
    // }
      ...
      >
        <Box m="lg" mt="xl">
          <Text mb="sm">
            ...
            function should return a 'score' for that response, which usually is
            a number or a boolean value (strings as categoricals are supported,
            but experimental).
          </Text>
          <Text mt="sm" mb="sm">
          ...
          <Text mt="md">
            ...
            possible, see the{" "}
            <a
              href="https://github.com/ianarawjo/ChainForge/blob/main/GUIDE.md#python-evaluator-node"
              target="_blank"
            >
              documentation
            </a>{" "}
